...
function cadastrarDependente(): void {
  ...
  let cpf: string = input("Digite o cpf: ");
  let idade: number = Number(input("Digite a idade: "));
  let nome: string = input("Digite o nome: ");
  let cpf_socio: string = input("Digite o cpf do socio: ");
  let owner: SocioFamilia = clube.consultarSocio(cpf_socio);

  let dependente = new Dependentes(cpf, idade, nome, owner);

  if (idade > 21) {
    throw new IdadeException(
      "Nao permitimos dependentes com idade acima de 21!"
    );
  }
  try {
    const socioAtual = clube.exibirSocio(cpf_socio)
    if (socioAtual instanceof SocioIndividual) {
      throw new EhIndividualException("Socio individual nao pode ter dependente!");
    }
  } catch (e: any) {
    console.log(e.message);
  }
...
function alterarSocio(): void {
  console.log("\nCadastrar Socio:\n");
  let cpfAntigo: string = input("Digite o cpf do socio que deseja editar: ");
  let cpf: string = input("Digite o cpf: ");
  let nome: string = input("Digite o nome: ");
  let senha: string = input("Digite a senha: ");
  ...
  clube.editarSocio(cpfAntigo, socio);
}
function alterarDependente(): void {
  ...
  clube.editarDependentes(cpfAntigo, dependente);
}
...
function exibirSocio(cpf: string) {
  console.log(clube.exibirSocio(cpf));
}

function exibirDependente(cpf: string) {
  console.log(clube.exibirDependente(cpf));
}
